    ...
    <table cellspacing="0">
      <thead>
        <tr>
          ...
          <?php include_partial('block/list_th_stacked', array('sort' => $sort)) ?>
      ...
      <tbody>
        <?php foreach ($pager->getResults() as $i => $block): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr class="sf_admin_row <?php echo $odd ?>">
            ...
            <?php include_partial('block/list_td_stacked', array('block' => $block)) ?>
            <?php include_partial('block/list_td_actions', array('block' => $block, 'helper' => $helper)) ?>
          </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
